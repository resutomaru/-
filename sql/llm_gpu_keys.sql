...
--   РАЗНЫЕ карты. Определи по ВСЕМУ тексту (заголовок, поле, описание), какая РЕАЛЬНО продаётся.
--   Используй ОБЪЁМ ПАМЯТИ как проверку: если заявленный объём невозможен для названной модели — название
--   неверно (напр., RTX 3060 Ti бывает только 8GB, значит «3060 Ti 12GB» — это RTX 3060). Выбирай кандидата,
--   чей объём физически сходится.
