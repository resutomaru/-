...
-- PREM-ТИР — ПРОБОВАЛИ И ОТКАТИЛИ (2026-06-20): гипотеза «ddr4-16gb медиана 6490 вздута премиумом»
--   НЕ подтвердилась. Зонд: экзамен стр.64 (новьё/магазин в источнике медианы) = 0 → медиана уже по
--   чистому used-private → 6490 ≈ реальный б/у рынок (DDR4 подорожала после EOL), не инфляция. После
--   разбиения дженерик 6000 ≈ премиум 7000 (бренд не разделяет), и тир раздробил зрелый K>20 (21) на
--   три K8-20-low → откат. §4 остаётся ГРУБЫМ (меньше бакетов = быстрее зреют). Не «чинить» повторно.
