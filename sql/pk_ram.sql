...
--   R3 (11.06, пакет №2): частота понимает кириллицу (МГц/МТ/с, в т.ч. склеенно «3200мгц»);
--     bare-allowlist дополнен DDR5-частотами 6800–8400 (раньше такие лоты теряли частоту → раскол бакета).
...
begin
  ...
  spd := coalesce( (regexp_match(s,'(\d{3,4})\s*(?:mhz|мгц|mt/?s|мт/?с|hz|гц)'))[1],   -- +голое hz/гц: «2666Hz» иначе теряло частоту → no-speed бакет (E19 чистка ddr4-16gb)
