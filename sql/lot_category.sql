-- lot_category — категоризатор как ФУНКЦИЯ (та же логика, что v2.3-канон в sql/categorizer.sql).
-- АУДИТ 2026-06-10 (приёмник): обёрнут в функцию для переиспользования в авто-нормализаторе (pg_cron).
-- ЛОГИКА идентична развёрнутому категоризатору v2.3 (валидирован превью-диффом 639/0 + 6 протечек).
-- DRIFT-CAVEAT: канон правил — здесь; one-shot в sql/categorizer.sql теперь = `update lots set
--   item_category = lot_category(title, model);`. При изменении правил менять ТОЛЬКО эту функцию.
