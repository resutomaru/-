...
### 3.4 Веб-кабинет
Supabase Auth (на старте — Telegram Login Widget или magic-link; детали — на этапе фронта). Маппинг auth-uid →
`client_id` через `client_identities (provider='supabase')`.
...
- **Бот ужимается до приятного:** уведомления о находках (карточка + кнопка «Открыть приложение»/«Купил»),
  быстрые команды (`/журнал` сводка). Тяжёлое редактирование сделок уезжает в **mini-app** (там и валидация, и UX).
  Это убирает хрупкий разбор команд с голыми id (причина нынешнего «молчит»).
...
1. **Онбординг нового клиента:** когда TG-личность не найдена в `client_identities` — (а) авто-создавать клиента
   на лету, или (б) только по приглашению/коду (контроль доступа)? *Влияет на §3.3.*
2. **Роль бота:** ужимаем до уведомлений + «Открыть приложение» (всё редактирование — в mini-app), или бот тоже
   умеет полноценно править журнал командами/кнопками? *Влияет на объём §10.*
3. **Веб-кабинет на старте:** нужен сразу, или mini-app + Sheets закрывают потребность, а веб — позже? *Влияет на §3.4.*
4. **Удаление сделки:** жёстко `delete` или мягко `archived` (с возможностью восстановить)? *Влияет на §6/§7.*
5. **Хостинг mini-app** (статика): Supabase Storage / Vercel / GitHub Pages — решим на этапе фронта (здесь не блок).

---

## Лог
- 2026-06-20 — чертёж v1 заведён (draft): архитектура, auth (TG initData + Supabase Auth), RLS-модель,
  модель данных, RPC-поверхность с контрактом ошибок, домен, Sheets-мост, наблюдаемость, миграция пилонами.
  Ждёт решений §12.
