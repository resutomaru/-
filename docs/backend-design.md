...
## Статус реализации (20.06) — BACKEND-API ЗАКРЫТ ✅
Готово и проверено на живой БД:
- **1a** идентичность — `clients`, `client_identities`, `invite_codes`, `current_client()`.
- **1b** RLS-изоляция — клиент видит только своё (тест: свой 1 / чужой 0).
- **1c** вход — `verify_initData` (**план Б**: проверка Telegram-подписи на КАЖДОМ вызове, БЕЗ JWT-секрета —
  он в новой Supabase недоступен; RLS остался вторым поясом). Заменяет §3.3 (Edge-Function-JWT не делаем).
- **2 / 2b** API-поверхность — `me`, `journal_list/add/update/delete/mark_sold/dashboard`, `find_buy`,
  `finds_list/get`, `config_get/update`. Схема: verify→cid→только своё, конверт `{ok,data}` / `{ok:false,error}`.

Дальше по backend: пилон 3 (домен-крепость), 4 (Sheets-мост), 5 (наблюдаемость) — **параллелятся с фронтом**.
Следующий крупный этап — **ФРОНТ** (mini-app + веб-кабинет + бот + Sheets), отдельным чертежом.
