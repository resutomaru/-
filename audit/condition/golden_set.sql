...
-- ---- E18-дочёс working-направления (11.06): РЕАЛЬНЫЕ строки из зонда по 893 working ----
-- Идемпотентный пересев только этого блока (прочие source='real' не трогаем).
delete from condition_golden where source='real' and err='E18';
insert into condition_golden (title, descr, expected, danger, err, source, note) values
('Видеокарта rtx 2060 6gb gigabyte','Продаю rtx 2060 с дефектом, при запуске стресс тестов и игр в моменте вылетают. Конкретная проблема в видеопамяти, чтобы нормально работала нужна полная замена на исправную. Изображение есть.','dead',true,'E18','real','зонд: дефект+вылеты+замена памяти; «нормально работала» НЕ спасает'),
('Rx 580 8gb 2048sp','Видеокарта работает, но в некоторых играх или при сильных нагрузках может вылетать.','dead',true,'E18','real','зонд: «работает, но вылетает» — позитив не спасает'),
('Оперативная память ddr3 ddr2 одним лотом','Часть памяти DDR3 рабочая, часть не определяется, какие-то планки на одних платах работают, на других нет.','dead',true,'E18','real','зонд: «не определяется» (смешанный лот; из медианы и так исключён is_component)'),
('Лот дисков и SSD','SSD SP 240Gb не определяется. WD Blue 1Tb щёлкает, не определяется.','dead',true,'E18','real','зонд: «не определяется»'),
...
-- ---- Пакет №3 (зонд unknown 11.06): U-серия — синтетика на новые словарные ветки ----
insert into condition_golden (title, descr, expected, danger, err, note) values
('Видеокарта RX 580','Вентиляторы крутят, изображения нет','dead',true,'U','инверсия «нет изображения» — порядок слов'),
('Материнская плата','Продаю под ремонт','dead',true,'U','«под ремонт» без слова «требует»'),
('Видеокарта','Карта не проходит стресс тест, в остальном работает','dead',true,'U','заявленный провал теста бьёт позитив (ШАГ 1)'),
('Процессор Ryzen 5','Не тянет современные игры','unknown',false,'U','«не тянет» — НЕ позитив (лукбихайд) и НЕ dead: слабый ≠ сломанный'),
('SSD Kingston 480GB','Пломба не оторвана, полностью рабочий','working',false,'U','лукбихайд: «не оторвана» — не дефект'),
('Блок питания 600W','Никаких проблем, не греется и не шумит','working',false,'U','отрицательные позитив-конструкции (только ШАГ 6)'),
('Видеокарта GTX 1070','Ремонта не требует, отдаю недорого','unknown',false,'U','инверсия «не требует ремонта» НЕ должна давать dead');

-- ---- Пакет №3: РЕАЛЬНЫЕ строки из зонда unknown (фрагменты; идемпотентный пересев блока) ----
delete from condition_golden where source='real' and err='U';
insert into condition_golden (title, descr, expected, danger, err, source, note) values
('Видеокарта 3070ti','Картинку выдает но по подгрузке драйверов уходит в бесконечную перезагрузку','dead',true,'U','real','3148558306: цикл-перезагрузка бьёт «картинку выдаёт»'),
('Ssd m2 1tbx5 1 лотом','У все дисков красный смарт, диски определяются','dead',true,'U','real','3149003454: красный SMART — доменный сигнал смерти'),
('rtx 3060 12 GB','Полностью проходит стресс тесты без нареканий но все равно нужно обслуживание и ремонт','dead',true,'U','real','3148861968: «нужно обслуживание и ремонт» бьёт стресс-тест-позитив'),
('SSD SP 240Gb','Sata3, потом перестал читатся. На другом компе читается','dead',true,'U','real','3148577391: «перестал читатся» (опечатка), интермиттент = dead'),
('aerocool vx plus 500w','Живой блок питания, небольшой торг есть','working',false,'U','real','3148845962: сленг «живой блок»'),
('Материнская плата с процессором','Проц и материнка не сочитываются, из-за этого изображения не будет','unknown',false,'U','real','3148857854: ЛОВУШКА — несовместимость пары ≠ дефект («изображения не будет» ≠ «изображения нет»)'),
('Блок питания Qdion qd500','Продаю ибо не справляется с нагрузкой на новом пк','unknown',false,'U','real','3147864768: ЛОВУШКА — слабый ≠ сломанный'),
('Процессор g4900 1151v2','Для чего подойдёт: апгрейд офисного пк, замена вышедшего из строя процессора','unknown',false,'U','real','3148216659: ЛОВУШКА — «вышедший из строя» = назначение товара, не состояние');
